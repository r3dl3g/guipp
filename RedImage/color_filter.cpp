...
namespace proc {
  ...
  {}
  //-----------------------------------------------------------------------------
  cv::Mat color_filter::calc (const cv::Mat& hsv_image, bool erode) const {
    cv::Mat mask[8];
    return calc(hsv_image, mask, erode);
  }
  //-----------------------------------------------------------------------------
  float color_filter::calc_portion (const cv::Mat& mask) {
    int mask_count = cv::countNonZero(mask);
    int full_count = mask.cols * mask.rows;
    return (float)((double)mask_count / (double)full_count);
  }
  //-----------------------------------------------------------------------------
  cv::Mat color_filter::calc (const cv::Mat& hsv_image, cv::Mat mask[8], bool erode) const {
