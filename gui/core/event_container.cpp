...
namespace gui {
  ...
  namespace core {
    ...
    bool event_container::handle_event (const event& ev, gui::os::event_result& resultValue) const {
      ...
      for (size_type i = 0; i < event_handlers.size(); ++i) {
        try {
          ...
          log_fatal << "exception in event_container::handle_event: " << e;
        } catch (...) {
          log_fatal << "Unknown exception in event_container::handle_event()";
        }
      }
