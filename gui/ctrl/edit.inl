...
namespace gui {
  ...
  namespace ctrl {
    ...
    template<text_origin_t origin, draw::frame::drawer frame, os::color F, os::color B>
    inline void basic_edit<origin, frame, F, B>::paint (draw::graphics& graph) {
      auto area = frame(graph, client_geometry());
      area.shrink(core::size::one);
      look::edit_line(graph, area, data.text, draw::font::system(), F, B, origin,
    ...
    template<text_origin_t origin, char character, draw::frame::drawer frame, os::color F, os::color B>
    inline void basic_pass<origin, character, frame, F, B>::paint (draw::graphics& graph) {
      auto area = frame(graph, client_geometry());
      area.shrink(core::size::one);
      std::string t(data.text.length(), character);
      look::edit_line(graph, area, t, draw::font::system(), F, B, origin,
                      data.selection, data.cursor_pos, data.scroll_pos, is_focused(), is_enabled());
    }
