...
namespace gui {
  ...
  namespace layout {
    ...
    core::rectangle split_view_traits<orientation_t::vertical>::get_first_place (const core::rectangle& sz,
                                                                                 double pos) {
      const auto w = core::point::type(sz.width() * pos - get_splitter_width() / 2);
    ...
    core::rectangle split_view_traits<orientation_t::vertical>::get_second_place (const core::rectangle& sz,
                                                                                  double pos) {
      const core::point::type x = sz.x() + core::point::type(sz.width() * pos + get_splitter_width() / 2);
      return sz.with_horizontal(x, sz.x2() - x);
    }

    template<>
    core::rectangle split_view_traits<orientation_t::vertical>::get_splitter_place (const core::rectangle& sz,
                                                                                  double pos) {
      const core::point::type x = sz.x() + core::point::type(sz.width() * pos - get_splitter_width() / 2);
      return sz.with_horizontal(x, get_splitter_width());
    }

    template<>
    core::rectangle split_view_traits<orientation_t::horizontal>::get_first_place (const core::rectangle& sz,
                                                                                   double pos) {
      const auto h = core::point::type(sz.height() * pos - get_splitter_width() / 2);
    ...
    core::rectangle split_view_traits<orientation_t::horizontal>::get_second_place (const core::rectangle& sz,
                                                                                    double pos) {
      const core::point::type y = sz.y() + core::point::type(sz.height() * pos + get_splitter_width() / 2);
      return sz.with_vertical(y, sz.y2() - y);
    }

    template<>
    core::rectangle split_view_traits<orientation_t::horizontal>::get_splitter_place (const core::rectangle& sz,
                                                                                    double pos) {
      const core::point::type y = sz.y() + core::point::type(sz.height() * pos - get_splitter_width() / 2);
      return sz.with_vertical(y, get_splitter_width());
    }
