 ...
 * Project    gui++ lib
 *
 * @brief     generic drawing functions
 *
...
namespace gui {
  ...
  namespace draw {
    ...
    void arc_or_pie<arc_type::arc>::operator() (graphics& g,
      ...
      draw_arc<arc_type::arc>(g, arc_coords(center, radius, start_angle, end_angle), p);
    }

    template<>
    void arc_or_pie<arc_type::arc>::operator() (graphics& g,
                                                const brush& b) const {
      fill_arc<arc_type::arc>(g, arc_coords(center, radius, start_angle, end_angle), b);
    }
    ...
    void arc_or_pie<arc_type::pie>::operator() (graphics& g,
                                                const pen& p) const {
      Use<brush> br(g, brush::invisible);
      draw_arc<arc_type::pie>(g, arc_coords(center, radius, start_angle, end_angle), p);
    }

    template<>
    void arc_or_pie<arc_type::pie>::operator() (graphics& g,
                                                const brush& b) const {
      arc_coords c(center, radius, start_angle, end_angle);
      pen pn(b.color());
      Use<pen> upn(g, pn);
    ...
    void draw_image_frame (graphics& g,
                           ...
                           const datamap<T>& img,
                           const core::native_rect& frame) {
      if ((rect.size() <= core::size::two) || !rect.is_valid()){
        return;
      }
