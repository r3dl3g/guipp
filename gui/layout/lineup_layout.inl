...
namespace gui {
  ...
  namespace layout {
    ...
    inline lineup_layout<H, D, B, G, S, R>::lineup_layout (const std::vector<layout_element>& list)
      : super(list)
    {}
    ...
    void lineup_layout<H, D, B, G, S, R>::layout (const core::rectangle& r) const {
      ...
      if (count) {
        if (space > 0) {
          ...
          const type offset = dimension + gap;
          const type sep_offset = gap + separatior_width;

          core::rectangle area = super::init_area(border_width, offset_dim1, offset_dim2,
