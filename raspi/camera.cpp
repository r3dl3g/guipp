...
#     define MMAL_CAMERA_CAPTURE_PORT 2
    ...
    // --------------------------------------------------------------------------
    std::ostream& operator<< (std::ostream& out, const still& cam) {
      cam.show_current(out);
      return out;
    }

    std::ostream& operator<< (std::ostream& out, const still::awb_gains& gains) {
      out << "{red:" << gains.r_gain << ", blue:" << gains.b_gain << "}";
      return out;
    }

    std::istream& operator>> (std::istream& in, still::awb_gains& gains) {
      char delemiter;
      in >> gains.r_gain >> delemiter >> gains.b_gain;
      return in;
    }

    std::ostream& operator<< (std::ostream& out, const still::color_fx& v) {
      out << "{enable:" << v.enable << ", u:" << v.u << ", v:" << v.v << "}";
      return out;
    }

    std::ostream& operator<< (std::ostream& out, const still::stereo_mode& v) {
      out << "{mode:" << v.mode << ", decimate:" << v.decimate << ", swap_eyes:" << v.swap_eyes << "}";
      return out;
    }

    std::ostream& operator<< (std::ostream& out, const still::size& v) {
      out << v.width << ", " << v.height;
      return out;
    }

    std::istream& operator>> (std::istream& in, still::size& sz) {
      char delemiter;
...
#       define mkAWBModeName(a) { still::AWBMode::a, #a }
    ...
    std::ostream& operator<< (std::ostream& out, const still::AWBMode& m) {
      if (m <= still::AWBMode::Greyworld) {
    ...
    std::istream& operator>> (std::istream& in, still::AWBMode& m) {
      std::string str;
      in >> str;
      if (str.size() > 0) {
    ...
    // --------------------------------------------------------------------------
    void still::enable () {
      LogTrace << "raspi::camera::still::enable()";
      ...
      m_null_preview.create(MMAL_COMPONENT_DEFAULT_NULL_SINK);
    ...
    // --------------------------------------------------------------------------
    void still::disable () {
      LogTrace << "raspi::camera::still::disable()";
    ...
    // --------------------------------------------------------------------------
    core::port still::get_still_output_port () const {
      return m_camera.still_port();
    }

    core::port still::get_control_port () const {
      return m_camera.control_port();
    }

    // --------------------------------------------------------------------------
    void still::capture () {
      LogTrace << "raspi::camera::still::capture()";
      m_camera.still_port().capture();
    }

    // --------------------------------------------------------------------------
    void still::set_defaults (uint32_t speed, uint32_t iso) {
      set_saturation(0);
      set_sharpness(0);
      set_contrast(0);
      set_brightness(0.5F);
...
//        m_camera.control_port().set(change_event_request.hdr);
    ...
    void still::show_current (std::ostream& out) const {
...
//        out << ", camera_rx_config:" << get_camera_rx_config();
    ...
    // --------------------------------------------------------------------------
    void still::set_rational (uint32_t id, float v, int den) {
      m_camera.control_port().set_float(id, v, den);
    }

    void still::set_rational (uint32_t id, float v, float min, float max, int den) {
      if ((min <= v) && (v <= max)) {
    ...
    auto still::get_awb_gains () const -> awb_gains {
      MMAL_PARAMETER_AWB_GAINS_T param = {{MMAL_PARAMETER_CUSTOM_AWB_GAINS,sizeof(param)}, {0,0}, {0,0}};
    ...
    // --------------------------------------------------------------------------
    void still::set_rotation (int rotation) {
    ...
    int still::get_rotation () const {
      return m_camera.preview_port().get_int32(MMAL_PARAMETER_ROTATION);
    }

    // --------------------------------------------------------------------------
    void still::set_flip (MMAL_PARAM_MIRROR_T mode) {
    ...
    MMAL_PARAM_MIRROR_T still::get_flip () const {
...
#define USE_ES_VIDEOxx
    ...
    // --------------------------------------------------------------------------
    void still::set_input_crop (const core::crop& c) {
#ifdef USE_CROP
    ...
    core::crop still::get_input_crop () const {
#ifdef USE_CROP
    ...
    // --------------------------------------------------------------------------
    void still::set_abs_crop (const MMAL_RECT_T& c) {
//        m_camera.control_port().set_crop(c);
    ...
    MMAL_RECT_T still::get_abs_crop () const {
    ...
    // --------------------------------------------------------------------------
    void still::set_color_fx (const color_fx& c) {
    ...
    auto still::get_color_fx () const -> color_fx {
    ...
    // --------------------------------------------------------------------------
    void still::set_stereo_mode (const stereo_mode& sm) {
    ...
    auto still::get_stereo_mode () const -> stereo_mode {
...
//        mmal_port_parameter_get(m_camera->output[2], &stereo.hdr);
    ...
    // --------------------------------------------------------------------------
    void still::set_drc (MMAL_PARAMETER_DRC_STRENGTH_T strength) {
    ...
    MMAL_PARAMETER_DRC_STRENGTH_T still::get_drc () const {
    ...
    // --------------------------------------------------------------------------
    void still::set_size (const size& sz) {
      m_current_size = sz;
...
//        crop.height = sz.height;
    ...
    auto still::get_size () const -> size {
...
//        return {(uint32_t)crop.width, (uint32_t)crop.height};
    ...
    // --------------------------------------------------------------------------
    uint32_t still::get_pixel_per_line () const {
      return VCOS_ALIGN_UP(m_current_size.width, 32);
    }

    // --------------------------------------------------------------------------
    MMAL_PARAMETER_CAMERA_CONFIG_T still::get_camera_config () const {
...
//        m_camera.control_port().get(camConfig.hdr);
    ...
    void still::set_camera_config (const MMAL_PARAMETER_CAMERA_CONFIG_T& camConfig) {
...
//        }
    ...
    void still::config_camera (const size& max_still, const size& max_preview) {
    ...
    // --------------------------------------------------------------------------
    MMAL_PARAMETER_CAMERA_INFO_CAMERA_T still::get_camera_info () const {
      return still::get_camera_info(get_camera_num());
    }

    // --------------------------------------------------------------------------
    MMAL_PARAMETER_CAMERA_INFO_CAMERA_T still::get_camera_info (int num) {
      if (num < 0) {
        num = 0;
      }
      MMAL_PARAMETER_CAMERA_INFO_T config = still::get_config();
    ...
    // --------------------------------------------------------------------------
    MMAL_PARAMETER_CAMERA_INFO_T still::get_config () {
    ...
    auto still::get_sensor_size () const -> size {
    ...
    MMAL_PARAMETER_CAMERA_RX_CONFIG_T still::get_camera_rx_config () const {
    ...
    void still::set_camera_rx_config (MMAL_PARAMETER_CAMERA_RX_CONFIG_T cam_rx_config) {
    ...
    // --------------------------------------------------------------------------
    MMAL_PARAMETER_RESIZE_T still::get_resize () const {
    ...
    void still::set_resize (MMAL_PARAMETER_RESIZE_T resize) {
    ...
    // --------------------------------------------------------------------------
    void still::set_metering_mode (MMAL_PARAM_EXPOSUREMETERINGMODE_T mode) {
      set_mode<MMAL_PARAM_EXPOSUREMETERINGMODE_T>(mode);
    }

    MMAL_PARAM_EXPOSUREMETERINGMODE_T still::get_metering_mode () const {
      return get_mode<MMAL_PARAM_EXPOSUREMETERINGMODE_T>();
    }

    // --------------------------------------------------------------------------
    void still::set_exposure_mode (MMAL_PARAM_EXPOSUREMODE_T mode) {
      set_mode<MMAL_PARAM_EXPOSUREMODE_T>(mode);
    }

    MMAL_PARAM_EXPOSUREMODE_T still::get_exposure_mode () const {
      return get_mode<MMAL_PARAM_EXPOSUREMODE_T>();
    }

    // --------------------------------------------------------------------------
    void still::set_flicker_avoid_mode (MMAL_PARAM_FLICKERAVOID_T mode) {
      set_mode<MMAL_PARAM_FLICKERAVOID_T>(mode);
    }

    MMAL_PARAM_FLICKERAVOID_T still::get_flicker_avoid_mode () const {
      return get_mode<MMAL_PARAM_FLICKERAVOID_T>();
    }

    // --------------------------------------------------------------------------
    void still::set_awb_mode (AWBMode awb_mode) {
      set_mode<MMAL_PARAM_AWBMODE_T>(static_cast<MMAL_PARAM_AWBMODE_T>(awb_mode));
    }

    auto still::get_awb_mode () const -> AWBMode {
      return static_cast<AWBMode>(get_mode<MMAL_PARAM_AWBMODE_T>());
    }

    // --------------------------------------------------------------------------
    void still::set_image_fx (MMAL_PARAM_IMAGEFX_T imageFX) {
      set_mode<MMAL_PARAM_IMAGEFX_T>(imageFX);
    }

    MMAL_PARAM_IMAGEFX_T still::get_image_fx () const {
      return get_mode<MMAL_PARAM_IMAGEFX_T>();
    }

    // --------------------------------------------------------------------------
    void still::set_camera_num (int32_t num) {
      m_camera_num = num;
      set_mode<int32_t>(num);
    }

    int32_t still::get_camera_num () const {
...
namespace std {
  ...
  ostream& operator<< (ostream& out, const MMAL_PARAMETER_CAMERA_CONFIG_T& v) {
  ...
  ostream& operator<< (ostream& out, const MMAL_PARAMETER_CAMERA_INFO_CAMERA_T& v) {
    out << "{name:" << v.camera_name << ", lens:" << v.lens_present
        << ", w:" << v.max_width  << ", h:" << v.max_height << "}";
    return out;
  }

  ostream& operator<< (ostream& out, const MMAL_PARAMETER_CAMERA_RX_CONFIG_T& v) {
  ...
  ostream& operator<< (ostream& out, const MMAL_PARAMETER_RESIZE_T& v) {
  ...
  ostream& operator<< (ostream& out, const MMAL_VIDEO_FORMAT_T& v) {
    out << "{w:" << v.width << ", h:" << v.height
        << ", crop:" << v.crop<< ", framerate:" << v.frame_rate
        << ", par:" << v.par << ", fourcc:" << raspi::core::four_cc(v.color_space)
        << "}";
    return out;
  }

  ostream& operator<< (ostream& out, const MMAL_RESIZEMODE_T v) {
    switch (v) {
