...
#   define check_null_ptr(ptr) check_null_ptr_(ptr, __FILE__, __LINE__)
    ...
    four_cc::operator bool () const {
      return type.uint32 != 0;
    }

    four_cc::operator MMAL_FOURCC_T () const {
      return type.uint32;
    }

    bool four_cc::operator== (const four_cc& rhs) const {
    ...
    four_cc& four_cc::operator= (MMAL_FOURCC_T t) {
      type.uint32 = t;
      return *this;
    }

    // --------------------------------------------------------------------------
    crop::crop (float x_, float y_, float w_, float h_)
        : x(x_) , y(y_) , width(w_) , height(h_)
      {}

    // --------------------------------------------------------------------------
    semaphore::semaphore (const char *name, VCOS_UNSIGNED initial_count) {
      check_vcos_status(vcos_semaphore_create(&m_semaphore, name, initial_count));
    }
    ...
    bool semaphore::wait (VCOS_UNSIGNED timeout) {
...
namespace std {
  ostream& operator<< (ostream& out, const raspi::core::four_cc& fourcc) {
  ...
  istream& operator>> (istream& in, raspi::core::four_cc& fourcc) {
  ...
  ostream& operator<< (ostream& out, const raspi::core::crop& v) {
    out << "{x:" << v.x << ", y:" << v.y << ", w:" << v.width << ", h:" << v.height << "}";
    return out;
  }

  istream& operator>> (istream& in, raspi::core::crop& v) {
    char delemiter;
    in >> v.x >> delemiter >> v.y >> delemiter >> v.width >> delemiter >> v.height;
    return in;
  }

  ostream& operator<< (ostream& out, const MMAL_RECT_T& v) {
    out << "{x:" << v.x << ", y:" << v.y << ", w:" << v.width << ", h:" << v.height << "}";
    return out;
  }

  ostream& operator<< (ostream& out, const MMAL_RATIONAL_T& v) {
    out << v.num << "/" << v.den;
    return out;
  }
