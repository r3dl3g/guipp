...
typedef scaler<double, scaling::log, double, double> log_scale;
typedef scaler<double, scaling::symlog, double, double> symlog_scale;

void test_linear_scaler_0 () {
  linear_scale sc({0, 100}, {0, 100});
  for (double i = 0; i < 100; i += M_PI / 10.0) {
    EXPECT_ALMOST_EQUAL(sc(i), i);
  }
}
...
void test_linear_scaler_0a () {
  ...
  for (double i = -110; i < 110; i += M_PI / 10.0) {
    ...
    EXPECT_ALMOST_EQUAL(sc(i), expected);
  }
}
...
void test_linear_scaler_5a () {
  linear_scale sc({100, 200}, {0.1, 0.2});
  EXPECT_ALMOST_EQUAL(sc(100), 0.1);
  EXPECT_ALMOST_EQUAL(sc(150), 0.15);
  EXPECT_ALMOST_EQUAL(sc(200), 0.2);
...
void test_linear_scaler_6 () {
  ...
  EXPECT_ALMOST_EQUAL(sc(-100), 0.2);
  EXPECT_ALMOST_EQUAL(sc(0), 0.15);
  EXPECT_ALMOST_EQUAL(sc(100), 0.1);
}
...
void test_symlog_scale_fn_1 () {
  typedef detail::scale_fn<double, double, scaling::symlog, double> fn;
  EXPECT_ALMOST_EQUAL(fn::range({0.1, 100}), 8);
  EXPECT_ALMOST_EQUAL(fn::precalc({0.1, 100}), -4);
  
  EXPECT_ALMOST_EQUAL(fn::calc(100, 0.1), 4);
  EXPECT_ALMOST_EQUAL(fn::calc(10, 0.1), 3);
  EXPECT_ALMOST_EQUAL(fn::calc(1, 0.1), 2);
  EXPECT_ALMOST_EQUAL(fn::calc(0.1, 0.1), 1);
  EXPECT_ALMOST_EQUAL(fn::calc(0.09, 0.1), 0.9);
  EXPECT_ALMOST_EQUAL(fn::calc(0.01, 0.1), 0.1);
  EXPECT_ALMOST_EQUAL(fn::calc(0.005, 0.1), 0.05);
  EXPECT_ALMOST_EQUAL(fn::calc(0.0, 0.1), 0);
  EXPECT_ALMOST_EQUAL(fn::calc(-0.005, 0.1), -0.05);
  EXPECT_ALMOST_EQUAL(fn::calc(-0.01, 0.1), -0.1);
  EXPECT_ALMOST_EQUAL(fn::calc(-0.09, 0.1), -0.9);
  EXPECT_ALMOST_EQUAL(fn::calc(-0.1, 0.1), -1);
...
void test_symlog_scale_fn_2 () {
  typedef detail::scale_fn<double, double, scaling::symlog, double> fn;
  EXPECT_ALMOST_EQUAL(fn::range({0.01, 10}), 8);
  EXPECT_ALMOST_EQUAL(fn::precalc({0.01, 10}), -4);
  
  EXPECT_ALMOST_EQUAL(fn::calc(10, 0.01), 4);
  EXPECT_ALMOST_EQUAL(fn::calc(1, 0.01), 3);
  EXPECT_ALMOST_EQUAL(fn::calc(0.1, 0.01), 2);
  EXPECT_ALMOST_EQUAL(fn::calc(0.01, 0.01), 1);
  EXPECT_ALMOST_EQUAL(fn::calc(0.009, 0.01), 0.9);
  EXPECT_ALMOST_EQUAL(fn::calc(0.001, 0.01), 0.1);
  EXPECT_ALMOST_EQUAL(fn::calc(0.0005, 0.01), 0.05);
  EXPECT_ALMOST_EQUAL(fn::calc(0.0, 0.01), 0);
  EXPECT_ALMOST_EQUAL(fn::calc(-0.0005, 0.01), -0.05);
  EXPECT_ALMOST_EQUAL(fn::calc(-0.001, 0.01), -0.1);
  EXPECT_ALMOST_EQUAL(fn::calc(-0.009, 0.01), -0.9);
  EXPECT_ALMOST_EQUAL(fn::calc(-0.01, 0.01), -1);
...
void test_symlog_scale_fn_3 () {
  typedef detail::scale_fn<double, double, scaling::symlog> fn;
